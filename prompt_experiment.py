...
A:""",
    ...
    "react": """당신은 20년 경력의 연금 상담 전문가입니다. 아래 질문에 답한 뒤, 자신의 답변에 대한 신뢰도를 1~5로 평가하고 근거를 덧붙이세요.

질문: {question}

[답변]
""",
    "cove": """아래 질문에 답변할 때, 반드시 다음과 같은 단계로 진행하세요:
1. [사실 검증 단계] 질문과 관련된 핵심 사실을 하나씩 나열하세요. 각 사실에 대해 공식 문서, 법령, 정책 등 신뢰할 수 있는 근거를 밝혀주세요.
2. [추론 단계] 나열한 사실을 바탕으로 논리적으로 결론을 도출하세요.
3. [최종 결론] 단계별 근거와 추론을 바탕으로, 질문에 대한 최종 답변을 한 문장으로 정리하세요.

반드시 각 단계를 번호로 구분해 명확히 작성하십시오.
질문: {question}
""",
    "cok": """아래 질문에 답변할 때, 반드시 여러 전문가의 의견이나 공식 기관의 근거를 순차적으로 인용하세요.
- [전문가별 관점 제시] 예: 연금재정 전문가, 세무 전문가, 투자 자문가 등 분야별로 의견을 구분해 적으세요.
- [최종 종합] 마지막에는 이 관점들을 바탕으로 결론을 종합해 정리하세요.

반드시 전문가별로 제목 또는 구분을 명확히 표기하십시오.
질문: {question}
""",
        "persona": """당신은 20년 경력의 금융 컨설턴트입니다. 항상 친절하고 이해하기 쉬운 어조로, 복잡한 금융 용어도 초보자도 알아듣게 풀어서 설명합니다. 확인되지 않은 정보는 제공하지 마시고, 공식 통계를 인용하거나 모르면 모른다고 답하세요.
...
for q_idx, question in enumerate(questions, 1):
    for prompt_type, template in PROMPT_TEMPLATES.items():
        print(f"실험 시작: 질문#{q_idx} 프롬프트[{prompt_type}]")
        ...
        print(f"실험 완료: 질문#{q_idx} 프롬프트[{prompt_type}]")
